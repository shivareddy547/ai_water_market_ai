    ...
    async placeOrder(userId, payload) {
        ...
        for (let i = 0; i < subOrders.length; i++) {
            ...
            const supplierOrderEntry = {
                ...
                collectEmptyCan: so.depositTotal > 0,
            ...
            if (so.supplierId) {
                supplierUser = await User.findOne({
                    where: { id: so.supplierId, role: 'supplier' }
                });
            }
            if (!supplierUser && so.supplier) {
                supplierUser = await User.findOne({
            ...
            if (supplierUser) {
                supplierPushPromises.push(
                    (async () => {
                        try {
                            // Use findOne instead of findOrCreate to avoid unique constraint errors
                            ...
                            console.error(`Failed to push order to supplier ${supplierUser.id}:`, err);
                        }
                    })()
                );
            } else {
                console.error(`Supplier not found for sub-order: ${so.supplier} (ID: ${so.supplierId})`);
            }
        }
